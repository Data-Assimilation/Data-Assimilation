...
def EnKF(X_f, y, m, R, H):
    deflation = 1
    dX = X_f - np.array(list(X_f.mean(axis=0, keepdims=True))*m)
    dY = H.dot(dX)
    K = dX.dot(dY.T).dot(np.linalg.inv((dY.dot(dY.T))+ (m-1)*R))
    return X_f + deflation*K.dot(np.array(([list(y)]*m) + np.random.normal(0, 1, [m, len(y)])- H.dot(X_f)))
