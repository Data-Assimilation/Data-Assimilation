...
plt.savefig("Fig7_2.png",format = 'png', dpi=300)
plt.show()
# print("RMSEが最小になるのはBを", np.argmin(Fig1)*gap, "倍した時")
print "RMSE takes minimum when np.argmin(Fig1)*gap=" + str((np.argmin(Fig1)+1)*gap) #argminは最初の要素が最小だったら0を返すので+1しとかないといけない
print np.min(Fig1)

#定数倍したBを保存しておく
np.savetxt("B_Refine.txt", B*np.argmin(Fig1)*gap, delimiter=", ")

plt.colorbar(plt.imshow(B*np.argmin(Fig1)*gap,interpolation="nearest"))
plt.title("B_Refine")
plt.xlabel("row")
plt.ylabel("column")
plt.savefig("B_Refine.png",format = 'png', dpi=300)
plt.show()
