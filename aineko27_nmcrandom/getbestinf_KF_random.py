...
Fig1 = []
#nが除く数
for n in range(0,J): ##32以降は発散してしまう
    #%%
    error = []
    ...
    fileName = "bestinf_KF_" + str(n) + "_random.txt"
    np.savetxt('bestinf_KF_random/'+fileName, np.array([0.05+(np.argmin(error[:,1][~np.isnan(error[:,1])])*inf_gap)]))